export default {
    name: 'Arun BS',
    role: 'Software Engineer IV',
    expYr: '10+',
    objective: `Creative front-end and BFF(Backend For Frontend) developer with 10+ years of work experience in developing web applications and embedded applications as per W3 Standards. Am interested to learn and leverage my expertise in full-stack development and the Google Cloud Platform. Onsite experience(Japan) to discuss the architecture and planning for the upcoming projects. Good exposure to AGILE and SCRUM methodologies.`,
    address: `26/b, 13th Cross Road, Pragathi Layout, Doddanekundi, Bangalore - 560037`,
    email: 'bsarunmca@gmail.com',
    contactNo: '+91 - 9739185574',
    portfolio: 'https://bsarunmca.github.io/',
    socialLinkList: [{
            icon: 'linkedin',
            url: 'https://www.linkedin.com/in/arun-bs-43233275/',
        },
        { icon: 'github', url: 'https://github.com/bsarunmca' },
        {
            icon: 'stack-overflow',
            url: 'https://stackoverflow.com/users/3901457/arun',
        },
    ],
    skills: [{
            name: 'JavaScript, ES6 and latest',
            per: '90',
        },
        { name: 'TypeScript', per: '75' },
        {
            name: 'React JS',
            per: '80',
        },
        {
            name: 'NestJS',
            per: '60',
        },
        {
            name: 'Redux',
            per: '75',
        },
        {
            name: 'jQuery',
            per: '90',
        },
        {
            name: 'HTML, HTML5',
            per: '90',
        },
        {
            name: 'CSS, CSS3, SCSS',
            per: '90',
        },
        {
            name: 'MUI, Bootstrap',
            per: '85',
        },
        {
            name: 'Git, GitHub',
            per: '80',
        },
        {
            name: 'CI/CD',
            per: '70',
        },
        {
            name: 'Google Cloud',
            per: '65',
        },
        {
            name: 'Docker, Kubernetes, Argo CD',
            per: '65',
        },
        {
            name: 'AJAX, JSON',
            per: '90',
        },
        {
            name: 'Webpack',
            per: '80',
        },
        {
            name: 'Angular JS (1.x)',
            per: '70',
        },
    ],
    expDetails: [{
            yearFrom: 'Jul 2014',
            yearTo: ' Nov 2015',
            company: 'Sakhatech Information Systems Pvt. Ltd',
            role: 'Frontend Developer',
            responsibility: `Responsible for developing and maintaining web applications for clients in various industries, 
            focusing on front-end development.`,
            environment: 'HTML5, CSS3, Bootstrap 3.0, JQuery, AJAX, JSON, Facebook API, Google plus API, Twiter API, & Linkedin API',
            keyResponsibilities: [
                `Developed and implemented new features and functionalities based on client requirements.`,
                `Collaborated with cross-functional teams to design and deploy scalable solutions.`,
                `Implement responsive design principles and optimize applications for various devices and screen sizes.`,
                `Integrate RESTful APIs and third-party libraries to fetch and display dynamic content on web pages.`,
            ],
        },
        {
            yearFrom: 'Dec 2015',
            yearTo: 'Nov 2017',
            company: 'Payoda Technologies',
            role: 'Software Engineer',

            responsibility: `Responsible for developing and maintaining web applications for Arista CVP(CLOUDVISION), 
            CLOUDVISION is a central portal for managing all the network switches.`,
            environment: `HTML5, CSS3, Angularjs, jQuery, d3js, Requirejs, AJAX, JSON, NPM, Grunt, ESLint, Lesshint, JSInspect, Jasmine.`,
            keyResponsibilities: [
                `Developed user interfaces using HTML, CSS, d3js, and Angularjs frameworks based on client requirements.`,
                `Conducted code reviews and provided constructive feedback to improve code quality.`,
                `New tool exploration and implementation as per the standard, got a spot award`,
                `Fixing critical issues in the application and got a spot award for that.`,
            ],
        },
        {
            yearFrom: 'Nov 2017',
            yearTo: 'Dec 2020',
            company: 'Toshiba Software India Private Limted',
            role: 'Senior Software Engineer',

            responsibility: `As a senior frontend developer am responsible for developing and maintaining apps for embedded browsers for MFP (Multi-Functional printers). Collaborated closely with clients to improve performance. Traveled to Japan(Onsite) to discuss the architecture and prioritize tasks.
             `,
            environment: `ReactJS, Redux, ESLint, Python, HTML5, CSS3, jQuery, JavaScript, Webpack`,
            keyResponsibilities: [
                `Developed user interfaces using ReactJS & Redux for MFP which has an embedded browser`,
                `Review the legacy code thoroughly to understand its structure, functionality, and dependencies.`,
                `Identify areas of the code that need improvement, such as outdated syntax, poor performance, or lack of documentation.`,
                `Reduced application load times by 40% by converting legacy to react application`,
                `Traveled to Japan(Onsite) to discuss the architecture, prioritize tasks, and support for offsite members and establish a 
               communication bridge between Toshiba MSM and Toshiba TSIP.`,
            ],
        },
        {
            yearFrom: 'Dec 2020',
            yearTo: 'Present',
            company: 'NCR Corporation',
            role: 'Software Engineer IV',

            responsibility: `As the UI Lead, Responsible for developing and maintaining web applications for banking, 
            focusing on front-end and back-end(BFF) development. 
            I oversee the design and development of user interfaces for web and mobile applications, 
            ensuring high-quality user experiences that align with business objectives.`,
            environment: `NestJS, ReactJS, Redux, GCP, Docker, Kubernetes, ArgoCD, GitHub, Jest, Webpack, monorepo, monolith,
            HTML5, CSS3, JavaScript, JSON, MUI, GIT, etc...`,
            keyResponsibilities: [
                `Developed and implemented new features and functionalities for banking applications [Payment, Template, Contacts, and Report module, etc...]`,
                `Deployed containerized applications using Docker and Kubernetes on GKE clusters, using Kubernetes manifests and Helm charts to Automate configuration management and rollback strategies using Argo CD.`,
                `Designed and configured CI/CD pipelines using GitHub CI/CD to automate code integration, testing, and deployment across multiple environments.`,
                `Conducted code reviews and provided constructive feedback to improve code quality.`,
                `Experienced in implementing server-side rendering (SSR) for React.js applications, enhancing performance, and user experience.`,
                `Participated in Agile scrum meetings to track project progress and prioritize tasks.`,
            ],
        },
    ],
    education: [{
            college: 'Erode Arts and Science College',
            course: 'MCA - Master of Computer Application',
            year: '2010 -  2013',
        },
        {
            college: 'Jairam Arts & Science College',
            course: 'BSc - Maths with Computer Application',
            year: '2006 - 2009',
        },
    ],
    achievements: [{
            place: 'Toshiba',
            achievement: [
                'Worked in Japan for technical collaboration and establishing communication bridges between teams',
                'Award-winning project in a Tech fair at Toshiba: WebPanel Style Guide. This project is a toolkit for developing the MFP UI',
            ],
        },
        {
            place: 'Payoda',
            achievement: [
                'SPOT award for New tool exploration and helping the team',
                'SPOT award for fixing critical issues',
            ],
        },
    ],
};